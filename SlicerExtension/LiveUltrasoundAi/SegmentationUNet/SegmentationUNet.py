...
try:
    ...
    msg.setIcon(qt.QMessageBox.Information)
    msg.setText(
        "Installation of onnxruntime is needed for some modules.\nIt may take a few minutes, but it only needs to be done\
...
class SegmentationUNetWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
  ...
  """
    ...
    def __init__(self, parent=None):
        ...
        self.lastFpsUpdateTime = (
            0  # To prevent too fast (hard to see) label update on GUI
        )
    ...
    """
        ...
        if int(slicer.app.revision) < 30261:
            slicer.util.errorDisplay(
                f"This module requires Slicer core version\nSlicer-4.13.0-2021-09-26 or later.",
        ...
        if not hasattr(slicer.modules, "processes"):
            slicer.util.messageBox(
                "This modules requires ParallelProcessing extension. Install it and restart Slicer."
            )
            return
        ...
        self.addObserver(
            slicer.mrmlScene, slicer.mrmlScene.EndImportEvent, self.onSceneImportEnd
        )

        self.ui.modelPathLineEdit.connect(
            "currentPathChanged(QString)", self.onModelSelected
        )
        ...
        self.ui.inputSelector.connect(
            "currentNodeChanged(vtkMRMLNode*)", self.onInputImageSelected
        )
        self.ui.outputSelector.connect(
            "currentNodeChanged(vtkMRMLNode*)", self.onOutputImageSelected
        )
        self.ui.outputTransformComboBox.connect(
            "currentNodeChanged(vtkMRMLNode*)", self.onOutputTransformSelected
        )
    ...
    def onProcessToggled(self, checked):
        self._parameterNode.SetParameter(
            self.logic.USE_SEPARATE_PROCESS, "True" if checked else "False"
        )
    ...
    """
        ...
        self.ui.inputSelector.setCurrentNode(
            self._parameterNode.GetNodeReference(self.logic.INPUT_IMAGE)
        )
        self.ui.outputSelector.setCurrentNode(
            self._parameterNode.GetNodeReference(self.logic.OUTPUT_IMAGE)
        )
        self.ui.outputTransformComboBox.setCurrentNode(
            self._parameterNode.GetNodeReference(self.logic.OUTPUT_TRANSFORM)
        )
    ...
    """
        if self.ui.applyButton.checked == True:
            if (time.time() - self.lastFpsUpdateTime) >= self.fpsLabelCooldown_s:
                self.ui.feedbackLabel.text = "Prediction running at {:.1f} FPS".format(
                    self.logic.getFps()
                )
...
class LivePredictionProcess(Process):
    def __init__(self, scriptPath, volume, model_path, active=True):
        ...
        self.input_transform = np.identity(
            4
        )  # This will be updated from logic when the volume is updated
    ...
    def onStarted(self):
        ...
        self.write(
            input_len
        )  # Write the length of the input we have yet to recieve in the buffer
    ...
    def useProcessOutput(self, processOutput):
        try:
            ...
            print(processOutput)
            logging.info(
                "EOFError: Check log text file in Slicer installation folder for error messages logged in process script"
            )
            self.output = None
...
class SegmentationUNetLogic(ScriptedLoadableModuleLogic, VTKObservationMixin):
  ...
  """
    ...
    def setupProcess(self):
        parameterNode = self.getParameterNode()
        scriptFolder = slicer.modules.segmentationunet.path.replace(
            "SegmentationUNet.py", ""
        )
        scriptPath = os.path.join(
            scriptFolder, "Resources", "ProcessScripts", "LivePredictions.slicer.py"
        )

        inputImageNode = parameterNode.GetNodeReference(self.INPUT_IMAGE)
        if inputImageNode is None:
            logging.error(
                "Cannot start segmentation process with no input image specified"
            )
            return
        ...
        self.livePredictionProcess = LivePredictionProcess(
            scriptPath, imageArray, modelPath
        )
        self.livePredictionProcess.connect(
            "readyReadStandardOutput()", self.processOutputReady
        )

        def onLivePredictProcessCompleted():
            logging.info("Live Prediction: Process Finished")
            self.livePredictionProcess.disconnect(
                "readyReadStandardOutput()", self.processOutputReady
            )

        processLogic = ProcessesLogic(
    ...
    def setRealTimePrediction(self, toggled):
        ...
        if toggled == True:
            ...
            if useProcess:
                ...
                self.setupProcess()
            self.inputModifiedObserverTag = inputImageNode.AddObserver(
                slicer.vtkMRMLScalarVolumeNode.ImageDataModifiedEvent,
    ...
    def updatePredictionOnProcess(self):
        ...
        if imageTransformNode is not None:
            ...
            self.livePredictionProcess.input_transform = slicer.util.arrayFromVTKMatrix(
    ...
    def updatePrecitionOnMain(self):
        ...
        input_image = Image.fromarray(
            input_array[0, :, :]
        )  # image.width is M, image.height is F
        resized_input_array = np.array(  # np.array will be (F, M) again, but resize happens in (M, F) axis order
            input_image.resize(
                (
                    int(
                        input_image.width * self.slicer_to_model_scaling[1]
                    ),  # M direction (width on US machine)
                    int(
                        input_image.height * self.slicer_to_model_scaling[0]
        ...
        )  # Flip to trained sound direction
        resized_input_array = (
        ...
        output_array = np.flip(
            output_array, axis=0
        )  # Flip back to match input sound direction
        """
        ...
        if apply_logarithmic_transformation:
            ...
            output_array = (
                np.log10(np.clip(output_array, 10 ** (-e), 1.0) * (10 ** e)) / e
            )
